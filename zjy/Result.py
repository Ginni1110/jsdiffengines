...
def differential_test(result: HarnessResult) -> List[DifferentialTestResult]:
    if result is None:  # 测试结果已经存储过了
        return []
    performance_factor = 10  # 测试性能的性能因子不能设置为参数，因为精简时也会进行测试。
...
def performance_test(result: HarnessResult, bugs_info: List[DifferentialTestResult], performance_factor) \
    ...
    """
    注意：若改变进行性能测试的方案时，一定记得修改用例精简中性能相关的测试结果是否可精简部分的代码
    """
    # if the runtime is more than performance_factor times of the fastest engine's, it means that the engine may
